...
//DMC
bool CLangCodeExpander::LookupISO31661(const std::string& saught,
                                       std::string& returned,
                                       bool returnCode)
{
  returned = "";
  ...
  if (saught.size() == 2)
  {
    for (const auto& codes : RegionNames)
    {
      if (saught == codes.alpha2UC)
      {
        if (!returnCode)
        {
  ...
  if (saught.size() > 3)
  {
    for (const auto& codes : RegionNames)
    {
      ...
      if (StringUtils::ToLower(saught) == StringUtils::ToLower(codes.englishname))
      {
        if (!returnCode)
        {
